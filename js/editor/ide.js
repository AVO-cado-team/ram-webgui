let editor = null;
let prevFileEncodedContent = null;
...
function createEditor()
{
    editor = monaco.editor.create(document.getElementById('container'), {
        value: previewCode.join('\n'),
        language: 'ram'
    });

    monaco.editor.defineTheme('ram-theme', loadTheme());
...
function configureRAMLanguage()
{
    ...
    monaco.languages.setMonarchTokensProvider('ram', {
        keywords: keywords,
        tokenizer: {
            root: [
                [/@?[a-zA-Z][\w$]*/, { cases: { '@keywords': 'keyword', '@default': 'varialbe' } } ],
                // [/\d+/, 'number'],
                [/=[\d*]/, 'delimiter'],
                [/[*]/, 'pointer'],
                [/#.*/, 'comment'],
